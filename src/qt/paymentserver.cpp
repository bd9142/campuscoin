...
const int CAMPUSCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CAMPUSCOIN_IPC_PREFIX("campuscoin:");
const char* CAMPUSCOIN_REQUEST_MIMETYPE = "application/campuscoin-paymentrequest";
const char* CAMPUSCOIN_PAYMENTACK_MIMETYPE = "application/campuscoin-paymentack";
const char* CAMPUSCOIN_PAYMENTACK_CONTENTTYPE = "application/campuscoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(CAMPUSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // campuscoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CAMPUSCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(CAMPUSCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click campuscoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start campuscoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling campuscoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on campuscoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in campuscoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(CAMPUSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // campuscoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom campuscoin addresses are not supported
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", CAMPUSCOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, CAMPUSCOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", CAMPUSCOIN_PAYMENTACK_MIMETYPE);
