...
CAMPUSCOIND=${1}/campuscoind
CLI=${1}/campuscoin-cli
...
B4ARGS="-datadir=$D4"
$CAMPUSCOIND $CAMPUSCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$CAMPUSCOIND $CAMPUSCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} ccc"
...
function StartThree {
  $CAMPUSCOIND $CAMPUSCOINDARGS $B1ARGS &
  B1PID=$!
  $CAMPUSCOIND $CAMPUSCOINDARGS $B2ARGS &
  B2PID=$!
  $CAMPUSCOIND $CAMPUSCOINDARGS $B3ARGS &
  B3PID=$!
}
